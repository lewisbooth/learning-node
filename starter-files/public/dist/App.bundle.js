  ...
  _axios2.default.get('/api/stores/near?lat=' + lat + '&lng=' + lng).then(function (res) {
    ...
    markers.forEach(function (marker) {
      marker.addListener('click', function () {
        var html = '\n              <div className=\'popup\'>\n                <a href=\'/store/' + this.place.slug + '\'>\n                  <img src=\'/uploads/' + (this.place.photo || "store.png") + '\' alt=\'' + this.place.name + '\' width="300"/>\n                  <p>' + this.place.name + ' - ' + this.place.location.address + '</p>\n                </a>\n              </div>\n            ';
...
function typeAhead(search) {
  ...
  searchInput.on('input', function () {
    ...
    _axios2.default.get('/api/search?q=' + this.value).then(function (res) {
      if (res.data.length) {
        ...
        _dompurify2.default.sanitize(searchResults.innerHTML = '<div class="search__result">There were no results for ' + _this.value + '</div>');
      }
    }).catch(function (err) {
      console.log(err);
    });
  ...
  searchInput.on('keydown', function (e) {
    ...
    var resultList = document.querySelectorAll('.search__result');
    var selectedIndex = resultList.findIndex(function (item) {
